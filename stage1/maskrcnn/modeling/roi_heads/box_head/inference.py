...
class PostProcessor(nn.Module):
    ...
    def forward(self, x, boxes):
        ...
        """
        class_logits, box_regression = x # torch.Size([2400,5]), torch.Size([2400,20])
        class_prob = F.softmax(class_logits, -1) # torch.Size([2400,5])
        ...
        proposals = list(proposals.split(boxes_per_image, dim=0)) # 300, 20
        class_prob = list(class_prob.split(boxes_per_image, dim=0)) # 300 5
        class_logits = list(class_logits.split(boxes_per_image, dim=0)) # 300 5

        results = []
        for i in range(len(boxes)): # batch boxes i 
    ...
    def prepare_boxlist(self, boxes, scores, image_shape, class_logits, gt_idxs):
        ...
        """
        # boxes : proposal # 300, 20 
        # scores : class_prob # 300, 5 (after softmax)
        # image_shape : bbox size, 300
        # class_logits : class_logits before softmax # 300, 5
        # gt_idxs : None

        ncls = scores.shape[1] # 5
        boxes = boxes.reshape(-1, 4) #  (300 * ncls) * 4
                                    # 300 * ncls cls0, cls1, cls2, cls3, cls4,
