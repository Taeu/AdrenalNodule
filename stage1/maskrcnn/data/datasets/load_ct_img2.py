...
def load_prep_img(data_dir, imname, spacing, slice_intv, slice_idx, do_clip=False, num_slice=3, is_train=False):
    ...
    if spacing is not None:  # spacing adjust, will overwrite simple scaling
        im_scale = float(spacing) / 0.7  # mean or medien spacing value
    else:
    ...
    if im_scale != 1:
        ...
        new_im = np.zeros((512, 512, 9))
        new_im[:min(im.shape[0], 512), :min(im.shape[1], 512), :] = im[:min(im.shape[0], 512), :min(im.shape[1], 512),
                                                                    :]
