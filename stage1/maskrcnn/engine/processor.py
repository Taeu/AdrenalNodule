...
def train_model():
    ...
    print('datasets num_classes : ', datasets[0].num_classes)
    print('datasets class idx : ', datasets[0].classes)
...
def test_model(model, is_validation):
    ...
    if is_validation:
        ...
        dataset_names = cfg.DATASETS.VAL # 'DeepLesion_val' # val을 별도로 지정
        stage = 'val'
        if cfg.MODEL.TAG_ON: # false 신경 안써도 되고
    ...
    output_folders = [None] * len(dataset_names) # [None]
    if cfg.RESULTS_DIR: # 'results'
